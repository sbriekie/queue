import os, io, ssl, json, uuid, pytz, tempfile, threading, functools, secrets, re, imaplib, pymysql, edge_tts
...
def start_cleanup_thread():
    """Start background cleanup thread jika belum berjalan"""
    global _cleanup_thread
    
    with _cleanup_lock:
        if _cleanup_thread is None or not _cleanup_thread.is_alive():
            _cleanup_thread = threading.Thread(target=cleanup_desk_heartbeat, daemon=True)
            _cleanup_thread.start()
            current_app.logger.info("[CLEANUP] Background thread started")
    ...
    if desk_id:
        ...
        if desk and desk.status != RowStatus.DELETED:
            return render_template("desk_client.html", 
                                 ...
                                 desk_user=session.get('desk_user', 'Operator'),  # TAMBAH INI
                                 satker_display=g.kode_satker,  # TAMBAH INI untuk tampilan
                                 mode="operating")
        session.pop('desk_id', None)
        session.pop('desk_name', None)
        session.pop('desk_user', None)
...
def kiosk_page():
    ...
    from models import Satker  # Import Satker model
    
    # Query services untuk satker aktif
    ...
    # Query nama satker aktif
    stmt_satker = select(Satker).where(
        Satker.kode_satker == g.kode_satker,
        Satker.status == RowStatus.ACTIVE
    )
    satker = db.session.execute(stmt_satker).scalar_one_or_none()
    satker_name = satker.nama if satker else "BPS SATKER"
...
def api_videos_rotation(kode_satker):
    ...
    if not satker:
        return jsonify({
            "success": False,
            "error": "Satker tidak ditemukan"
        }), 404

    # Query dengan filter is_deleted = False (dan status ACTIVE jika ada)
    stmt = select(Video).where(
        Video.kode_satker == kode_satker,
        Video.is_deleted == False,           # Filter soft-delete
        # Video.status == RowStatus.ACTIVE   # Uncomment jika ada field status
    ).order_by(Video.created_at)

    videos = db.session.execute(stmt).scalars().all()

    # Return empty array jika tidak ada video (bukan error)
    if not videos:
        return jsonify({
            "success": True,
            "kode_satker": kode_satker,
        ...
        "videos": [
            {
                ...
                "title": getattr(vid, 'title', None) or getattr(vid, 'name', f"Video {i+1}"),
...
def replay_request():
    ...
    try:
        ...
        desk_id = data.get("desk_id") or session.get('desk_id')
        
        if not ticket or not desk_id:
            return jsonify({"error": "ticket & desk_id required"}), 400
        
        if session.get('desk_id') != int(desk_id):
            return jsonify({"error": "Not your assigned desk"}), 403
        ...
        return jsonify({"success": True, "ticket": ticket, "desk": desk.name})
        
    except Exception as e:
        current_app.logger.error(f"Error in replay_request: {str(e)}")
        return jsonify({"error": "Internal server error"}), 500

@app.route("/api/last-replay/<kode_satker>")  
def api_last_replay(kode_satker):  
    """Polling endpoint untuk display client"""
    global LAST_REPLAY  
    
    try:
        satker_data = LAST_REPLAY.get(kode_satker, {}) if isinstance(LAST_REPLAY, dict) else {}
        
        if not satker_data:
            return jsonify({"ticket": None, "timestamp": None})
        
        created = satker_data.get("created")
        if created and (datetime.now(pytz.UTC) - created).total_seconds() > 15:
            LAST_REPLAY.pop(kode_satker, None)
            return jsonify({"ticket": None, "timestamp": None})
        
        return jsonify({
            "ticket": satker_data.get("ticket"),
            "desk": satker_data.get("desk"),
            "desk_id": satker_data.get("desk_id"),
            "type": satker_data.get("type", "manual"),
            "kode_satker": kode_satker, 
            "timestamp": created.isoformat() if created else None
        })
        
    except Exception as e:
        current_app.logger.error(f"Error in api_last_replay: {str(e)}")
        return jsonify({"error": "Internal server error"}), 500
...
def desk_heartbeat():
    """
    Receive heartbeat dari desk client
    
    Logic:
    1. Update timestamp desk pengirim
    2. Aktifkan desk di DB jika sebelumnya inactive
    3. Cek semua desk lain: jika > HEARTBEAT_TIMEOUT = timeout, jika > GHOST_TIMEOUT = auto-deactivate
    """
    try:
        ...
        # 1. Update heartbeat untuk desk pengirim
        DESK_HEARTBEAT[desk_id] = {
            "last_seen": now_aware,
            "ip": request.remote_addr,
            "user": session.get('desk_user', 'unknown')
        }
        
        # 2. Pastikan desk aktif di database (reactivate jika perlu)
        current_desk = db.session.get(Desk, desk_id)
        if current_desk:
            if not current_desk.is_active:
                current_desk.is_active = True
                current_desk.updated_at = now_aware
                current_app.logger.info(f"[HEARTBEAT] Desk {current_desk.name} reactivated")
            
            # Update last_activity desk
            current_desk.last_activity = now_aware
            db.session.commit()
        
        # 3. CLEANUP: Cek semua desk untuk timeout & ghost
        ghost_deactivated = []
        timeout_desks = []
        
        for did, hb_data in list(DESK_HEARTBEAT.items()):
            last_seen = hb_data["last_seen"]
            ...
            time_diff = (now_aware - last_seen).total_seconds()
            
            # GHOST DETECTION: > 60 detik = Auto deactivate DB + hapus dari memory
            if time_diff > GHOST_TIMEOUT:
                desk = db.session.get(Desk, did)
                if desk and desk.is_active:
                    desk.is_active = False
                    desk.updated_at = now_aware
                    db.session.commit()
                    
                    ghost_deactivated.append({
                        'id': did,
                        'name': desk.name,
                    ...
                    current_app.logger.warning(
                        f"[GHOST] Desk {desk.name} auto-deactivated after {int(time_diff)}s"
                    )
                
                # Hapus dari heartbeat dict karena sudah di-deactivate
                DESK_HEARTBEAT.pop(did, None)
                
            # TIMEOUT DETECTION: > 10 detik = Tandai timeout (tapi DB tetap active, hanya tandai offline)
            elif time_diff > HEARTBEAT_TIMEOUT:
                timeout_desks.append({
                    'id': did,
                    'offline_seconds': int(time_diff)
                })
        ...
        return jsonify({
            ...
            "desk_status": {
                "id": desk_id,
                "is_active": current_desk.is_active if current_desk else False,
                "last_seen": now_aware.isoformat()
            },
            "cleanup_results": {
                "ghost_deactivated": ghost_deactivated,  # List desk yang di-set inactive karena >60s
                "timeout_offline": timeout_desks         # List desk yang timeout (>10s) tapi belum ghost
            }
        })
...
def api_new_ticket(service_id):
    ...
    """
    from models import TicketStatus, RowStatus, Ticket, KioskMenu, Satker
    ...
    with lock:
        try:
            ...
            # AMBIL NAMA SATKER BERDASARKAN KODE SATKER AKTIF
            stmt_satker = select(Satker).where(
                Satker.kode_satker == g.kode_satker,
                Satker.status == RowStatus.ACTIVE
            )
            ...
            t = Ticket(
                ...
                kode_satker=g.kode_satker,  # <-- INI YANG PENTING
                created_by=request.remote_addr or "kiosk"
            )
            db.session.add(t)
            db.session.commit()
            
            current_app.logger.info(f"Ticket created: {number} for satker: {g.kode_satker}")
            ...
            return jsonify({
                ...
                "service": svc.name,
                "satker_name": satker_name,
                "kode_satker": g.kode_satker,  # <-- PASTIKAN INI ADA
...
def analytics_desk_status():
    """Get real-time desk status - Hanya membaca status, tidak ada auto-deactivate"""
    ...
    HEARTBEAT_TIMEOUT = 10  # Deteksi offline (detik)
    
    # Query semua desk untuk satker ini
    stmt = select(Desk).where(
        Desk.kode_satker == kode_satker,
    ...
    for desk in desks:
        ...
        # Cek tiket yang sedang dilayani (SERVING)
        ...
        # Cek status online/offline dari heartbeat
        if desk.id in DESK_HEARTBEAT:
            hb_data = DESK_HEARTBEAT[desk.id]
            last_seen = hb_data["last_seen"]
            
            # Handle backward compatibility (naive -> aware)
            if last_seen.tzinfo is None:
                last_seen = pytz.UTC.localize(last_seen)
        ...
        elif last_seen:  # Pernah online tapi sekarang offline
            status_display = 'offline'
        else:  # Belum pernah kirim heartbeat
            status_display = 'never_connected'
    ...
    return jsonify({
        ...
        'meta': {
            ...
            'offline_count': sum(1 for d in result if not d['is_online'] and d['last_seen'])
        }
    })
...
@admin_required
def video_list():
    # Filter satker untuk superadmin
    selected_satker = g.kode_satker if not is_super_admin() else (request.args.get('satker') or g.kode_satker)

    # Gunakan query_active() untuk filter is_deleted=False otomatis
    videos = Video.query_active().filter_by(
        kode_satker=selected_satker
...
def video_add():
    ...
    video = Video(
        filename=url,
        kode_satker=kode_satker,
        created_by=g.user.username  # BaseModel field
    )
...
def video_edit(video_id):
    # Query hanya video yang aktif (tidak di-soft-delete)
    video = Video.query_active().filter_by(id=video_id).first()
    
    if not video:
        abort(404)
    
    # Cek authorization untuk non-superadmin
    if not is_super_admin() and video.kode_satker != g.kode_satker:
        abort(403)
    ...
    video.updated_by = g.user.username  # BaseModel field, updated_at auto-onupdate
    db.session.commit()
...
def video_delete(video_id):
    ...
    # Gunakan method soft_delete() dari BaseModel
    # Ini akan set: status=DELETED, is_deleted=True, deleted_at=now, deleted_by=user, updated_at=now
    video.soft_delete(deleted_by=g.user.username)
    db.session.commit()
...
def chart_add():
    try:
        ...
        kode_satker = (
            request.form.get("kode_satker", g.kode_satker)
            if is_super_admin()
            else g.kode_satker
        )
        ...
        if not judul or not slug:
            ...
            return redirect(url_for(
                "admin.chart_list",
                satker=kode_satker if is_super_admin() else None
            ))
        ...
        try:
            ...
            opsi_animasi = json.loads(opsi_animasi_str) if opsi_animasi_str else {
                "duration": 2000,
                "easing": "easeOutQuart"
            }
        ...
        # 1️⃣ Jika slug AKTIF → TOLAK
        existing_active = Chart.query.filter_by(
            kode_satker=kode_satker,
            slug=slug,
            is_deleted=False
        ).first()
        ...
        if existing_active:
            flash(f"Slug '{slug}' sudah digunakan dan masih aktif", "danger")
            return redirect(url_for(
                "admin.chart_list",
                satker=kode_satker if is_super_admin() else None
            ))

        # 2️⃣ Jika slug ADA tapi SOFT-DELETED → PULIHKAN
        existing_deleted = Chart.query.filter_by(
            kode_satker=kode_satker,
            slug=slug,
            is_deleted=True
        ).first()
        ...
        if existing_deleted:
            existing_deleted.judul = judul
            existing_deleted.tipe_chart = tipe_chart
            existing_deleted.data_json = data_json
            existing_deleted.opsi_animasi = opsi_animasi
            existing_deleted.refresh_detik = int(refresh_detik) if refresh_detik else 60
            existing_deleted.is_deleted = False
            existing_deleted.created_by = g.user.username

            db.session.commit()
            flash("Chart berhasil dipulihkan dan diperbarui", "success")
            return redirect(url_for(
                "admin.chart_list",
                satker=kode_satker if is_super_admin() else None
            ))

        # 3️⃣ Jika slug BENAR-BENAR BARU → INSERT
        chart = Chart(
            slug=slug,
            judul=judul,
...
def cleanup_desk_heartbeat():
    """Background thread untuk cleanup ghost desks - FIXED"""
    import time
    ...
    while True:
        try:
            with app.app_context():
                ...
                # STRATEGI 1: Cek desk di memory DESK_HEARTBEAT (ada heartbeat tapi timeout)
                for did, hb_data in list(DESK_HEARTBEAT.items()):
                    ...
                    # Ghost: >60 detik tidak kirim heartbeat
                    if time_diff > GHOST_TIMEOUT:
                        desk = db.session.get(Desk, did)
                        if desk and desk.is_active:
                            desk.is_active = False
                            ...
                            current_app.logger.warning(
                                f"[GHOST CLEANUP] Desk {desk.name} (ID:{did}) auto-deactivated after {int(time_diff)}s (from heartbeat memory)"
                            )
                        
                        # Hapus dari memory
                        DESK_HEARTBEAT.pop(did, None)
                
                # STRATEGI 2: Cek database untuk desk aktif tapi tidak ada di heartbeat (orphaned)
                # Ini untuk desk yang aktif di DB tapi client sudah lama mati/tidak pernah kirim heartbeat
                active_desks = db.session.execute(
                    select(Desk).where(
                        Desk.is_active == True,
                        Desk.status != RowStatus.DELETED
                    )
                ...
                for desk in active_desks:
                    # Cek apakah desk ini ada di heartbeat memory
                    if desk.id not in DESK_HEARTBEAT:
                        # Desk aktif di DB tapi tidak ada heartbeat sejak server start
                        # Cek last_activity di DB jika ada
                        last_activity = desk.last_activity
                        
                        if last_activity:
                            if last_activity.tzinfo is None:
                                last_activity = pytz.UTC.localize(last_activity)
                            
                            time_diff = (now_aware - last_activity).total_seconds()
                            
                            # Jika last_activity > GHOST_TIMEOUT, deactivate
                            if time_diff > GHOST_TIMEOUT:
                                desk.is_active = False
                                desk.updated_at = now_aware
                                db.session.commit()
                                ...
                                current_app.logger.warning(
                                    f"[GHOST CLEANUP] Desk {desk.name} (ID:{desk.id}) auto-deactivated (no heartbeat since server start, last activity {int(time_diff)}s ago)"
                                )
                        else:
                            # Tidak ada last_activity, cek created_at atau langsung nonaktifkan 
                            # jika memang tidak ada heartbeat sama sekali
                            # Opsional: biarkan saja atau nonaktifkan setelah waktu tertentu
                            pass
    ...
    with _cleanup_lock:
        if _cleanup_thread is None or not _cleanup_thread.is_alive():
            _cleanup_thread = threading.Thread(
                target=cleanup_desk_heartbeat, 
                daemon=True,
                name="DeskCleanupThread"
            )
            _cleanup_thread.start()
            # Gunakan print karena belum ada app context saat startup
            print("[CLEANUP] Background thread started")
            return True
        return False

def reset_all_desk_status():
    """Reset semua desk ke inactive saat server start"""
    with app.app_context():
        try:
            result = db.session.execute(
                select(Desk).where(Desk.is_active == True)
            ).scalars().all()
            
            for desk in result:
                desk.is_active = False
                desk.updated_at = datetime.now(pytz.UTC)
            
            db.session.commit()
            current_app.logger.info(f"[INIT] Reset {len(result)} desk(s) to inactive on server start")
        except Exception as e:
            db.session.rollback()
            print(f"[ERROR] Failed to reset desk status: {e}")
...
if __name__ == "__main__":
    ...
    # RESET: Nonaktifkan semua desk saat startup (bersihkan ghost dari sesi sebelumnya)
    reset_all_desk_status()
    ...
    print(f"[INFO] Heartbeat timeout: {HEARTBEAT_TIMEOUT}s, Ghost timeout: {GHOST_TIMEOUT}s")
